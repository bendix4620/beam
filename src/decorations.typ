...
#let resolve-decoration(ctx, dec, decor-type) = {
    ...
    let beam-style = ctx.beam.style
    beam-style.decoration.at(decor-type) = merge(beam-style.decoration.at(decor-type), if type(dec) == dictionary { dec } else { (content: dec) })

    let dec = resolve-style(beam-style).decoration.at(decor-type)
