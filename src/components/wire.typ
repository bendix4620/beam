...
#let wire(bits: 0, shape: "direct", ratio: 50%, axis: "x", i: none, name: none, ..params) = {
    ...
    group(name: name, ctx => {
        ...
        if i != none {
            ...
            beam-style.decoration.current.wire = merge(beam-style.decoration.current.wire, if type(i) == dictionary { i } else { (content: i) })

            let dec = resolve-style(beam-style).decoration.current.wire
            mark(
